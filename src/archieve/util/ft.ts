...
/**
 * Dynamic Fungible Token Properties - fixed supply is false
 */
...
provider = mxw.Wallet.fromMnemonic(nodeProvider.fungibleToken.provider).connect(
  providerConnection
);
issuer = mxw.Wallet.fromMnemonic(nodeProvider.fungibleToken.issuer).connect(
  providerConnection
);
middleware = mxw.Wallet.fromMnemonic(
  nodeProvider.fungibleToken.middleware
...
const run = async () => {
   ...
   */
  Logger.white("------------------------------------------");
  let fungibleToken: mxw.token.FungibleToken = await new Creator(
    issuer,
    fungibleTokenProperties
  ).createFixFT();
  ...
  var approveReceipt = await new Authorize(
    fungibleTokenProperties,
   ...
   */
  Logger.white("------------------------------------------");
  let dynFungibleToken: mxw.token.FungibleToken = await new Creator(
    issuer,
    dynFungibleTokenProperties
  ).createDynFT();
   ...
   */
  Logger.white("------------------------------------------");
  Logger.bold("Approving Dyanmic Fungible Token...");
  approveReceipt = await new Authorize(
    dynFungibleTokenProperties,
    token.FungibleToken.approveFungibleToken,
    issuer,
    provider,
   ...
   */
  Logger.white("------------------------------------------");
  dynFungibleToken = await new Query(
    dynFungibleTokenProperties,
    issuer,
    issuer
  ).QueryToken();
  ...
  var freezeReceipt = await new Authorize(
    dynFungibleTokenProperties,
    token.FungibleToken.freezeFungibleToken,
    issuer,
    provider,
    middleware
  ).approve();
  if (freezeReceipt.status == 1) Logger.green("Token is frozen!");
  else Logger.red("Failed to freeze!");
  dynFungibleToken = await new Query(
    dynFungibleTokenProperties,
    issuer,
    issuer
  ...
  var unfreezeReceipt = await new Authorize(
    dynFungibleTokenProperties,
    token.FungibleToken.unfreezeFungibleToken,
    issuer,
    provider,
    middleware
  ).approve();
  if (unfreezeReceipt.status == 1) Logger.green("Token is unfrozen!");
  else Logger.red("Failed to unfreeze!");
