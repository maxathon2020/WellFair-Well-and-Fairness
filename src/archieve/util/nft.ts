...
import { mxw, nonFungibleToken as token } from "mxw-sdk-js/dist";
...
wallet = mxw.Wallet.fromMnemonic(nodeProvider.kyc.issuer).connect(
  providerConnection
);
...
const run = async () => {
  ...
  const receipt = await new Approver(
    symbol,
    provider,
    issuer,
    middleware
  ...
  const transferReceipt = await new Transferer(symbol, itemId, issuer).transfer(
    wallet.address
  );
